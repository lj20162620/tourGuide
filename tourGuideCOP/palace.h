...
/*************************修改路径信息**********************/
void modifyPlace(){
   // creat(&g,"palacegra.txt");     //将图的信息取进来
...
void changeWays(int paT){
   ...
   // int newNum[paT];
    ...
    int aaa;
    if(sta>=0&&fin<paT){
           if(g.edges[sta][fin]!=FINAL||g.edges[fin][sta]!=FINAL){
                printf("此两点间存在路径！长度为：%d\n",g.edges[sta][fin]);
                printf("是否将该条路径关闭？ 1：关闭  0：离开\n");
                printf("%d %d\n",g.n,g.e);
                scanf("%d",&ch);
                  if(ch==1){
                                g.edges[sta][fin] = FINAL;g.e--;
                                newTotal[0]= g.n;
                                newTotal[1]= g.e;
                                printf("%d %d\n",newTotal[0],newTotal[1]);
                                scanf("%d",&aaa);
                                k=0;
                                    for(i=0;i<g.n;i++)
                        {                for(j=0;j<g.n;j++)
                                    {
                                        if(g.edges[i][j]!=FINAL||g.edges[i][j]!=0)
                                           {
                                                newgra[k].id1=i+1;
                                                newgra[k].id2=j+1;
                                                newgra[k].length=g.edges[i][j]; k++;
                                           }
                                    }
                        }
                                    printf("%d %d\n",newTotal[0],newTotal[1]);
                                    printf("%d %d\n",g.n,g.e);
                                    scanf("%d",&aaa);
                               saveArraytxt(newTotal,"palacegra.txt",2);
                             //  saveArraytxt(g.vexs,"palacegra.txt",g.n);
                              // saveStructtxt(newgra,"palacegra.txt",g.e);
...
void printgpd(Mgraph g,int p[],int d[],palace palaceArray[]){
